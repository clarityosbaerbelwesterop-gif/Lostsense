...
bool IsAllowed(DeepMechanismId id, DeepMechanismState state) noexcept {
    ...
    case DeepMechanismId::LanternRailSwitch:
        return state == DeepMechanismState::Locked || state == DeepMechanismState::Available ||
               state == DeepMechanismState::Primary || state == DeepMechanismState::Alternate;
...
bool DeepRouteState::Complete(DeepRouteMilestone milestone) {
    ...
    if (index > static_cast<std::uint32_t>(DeepRouteMilestone::PumpCathedralApproachOpened)) {
        return false;
    }
...
bool DeepRouteState::IsComplete(DeepRouteMilestone milestone) const noexcept {
    const auto index = static_cast<std::uint32_t>(milestone);
    return index <= static_cast<std::uint32_t>(DeepRouteMilestone::PumpCathedralApproachOpened) &&
           (CompletedMask_ & Bit(milestone)) != 0U;
}

bool DeepRouteState::SetMechanism(DeepMechanismId id, DeepMechanismState state) {
...
std::optional<DeepRouteState> DeepRouteState::Deserialize(std::string_view text) {
    ...
    for (std::size_t index = 0; index < values.size(); ++index) {
        ...
        if ((!final && separator == std::string_view::npos) || (final && separator != std::string_view::npos)) {
            return std::nullopt;
        }
...
bool DeepRouteState::ValidMilestoneOrder(std::uint32_t mask) noexcept {
    constexpr auto count = static_cast<std::uint32_t>(DeepRouteMilestone::PumpCathedralApproachOpened) + 1U;
    if ((mask >> count) != 0U) {
        return false;
    }
    bool foundGap = false;
...
bool DeepRouteState::ValidState(const DeepRouteState& state) noexcept {
    ...
    if (!state.IsComplete(DeepRouteMilestone::OdranDefeated) && state.RailSwitch_ != DeepMechanismState::Locked) {
        return false;
    }
    if (!state.IsComplete(DeepRouteMilestone::LanternRailDiscovered) && state.ServiceBrake_ != DeepMechanismState::Locked) {
        return false;
    }
    if (!state.IsComplete(DeepRouteMilestone::ServiceBrakeRestored) && state.PressureDoor_ != DeepMechanismState::Locked) {
        return false;
    }
    if (!state.IsComplete(DeepRouteMilestone::RoyalThresholdOpened) &&
        (state.VentilationIntake_ != DeepMechanismState::Locked || state.ReliefVent_ != DeepMechanismState::Locked)) {
        return false;
    }
    if (!state.IsComplete(DeepRouteMilestone::VentilationNaveStabilized) && state.ReturnShortcut_ != DeepMechanismState::Locked) {
        return false;
    }
    return true;
}
