...
AttributeSet CreateDefaultSet() {
  ...
  DefineOrAbort(attributes,
                {MovementSpeedMultiplier, 1.0, 0.0, 10.0});
  ...
  DefineOrAbort(attributes,
                {DamageBonusPercent, 0.0, -100.0, MaximumPercent});
  DefineOrAbort(attributes,
                {ArmorPenetration, 0.0, 0.0, MaximumMagnitude});
  ...
  for (std::size_t index = 0; index < Combat::DamageTypeCount; ++index) {
    ...
    DefineOrAbort(attributes,
                  {DamageBonus(type), 0.0, -100.0, MaximumPercent});
    DefineOrAbort(attributes, {Resistance(type), 0.0, -1.0, 1.0});
    DefineOrAbort(attributes,
                  {ResistancePenetration(type), 0.0, 0.0, 2.0});
  }
  return attributes;
}
