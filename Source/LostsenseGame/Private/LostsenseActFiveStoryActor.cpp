...
void ALostsenseActFiveStoryActor::Configure(const int32 InQuestId) {
  QuestId = InQuestId; RestScale = GetActorScale3D(); bConfigured = true; Sync();
}
FText ALostsenseActFiveStoryActor::GetInteractionPrompt() const {
  switch (QuestId) {
  case 80040: return FText::FromString(TEXT("Map the first Namarith district with Nera Quill"));
  case 80041: return FText::FromString(TEXT("Operate the civic consensus anchors"));
  case 80042: return FText::FromString(TEXT("Recover the First Witness protocol"));
  case 80043: return FText::FromString(TEXT("Open the Returned reconstruction chamber"));
  default: return FText::GetEmpty();
  }
}
bool ALostsenseActFiveStoryActor::CanInteract(const ALostsenseKnightCharacter &Interactor) const {
  ...
  return bConfigured && Story != nullptr &&
      ...
      FVector::DistSquared(GetActorLocation(), Interactor.GetActorLocation()) <= FMath::Square(320.0F);
}
bool ALostsenseActFiveStoryActor::Interact(ALostsenseKnightCharacter &Interactor) {
...
void ALostsenseActFiveStoryActor::Tick(const float DeltaSeconds) { Super::Tick(DeltaSeconds); Sync(); }
void ALostsenseActFiveStoryActor::Sync() {
  if (!bConfigured) return;
  SetActorScale3D(RestScale);
  const ULostsenseStorySubsystem *Story = StoryFor(*this);
  if (Story != nullptr && Story->GetObjectiveState(QuestId) == ELostsenseObjectiveState::Completed)
