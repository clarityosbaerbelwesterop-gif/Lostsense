...
FArchetypeTuning TuningFor(const ELostsenseEnemyArchetype Archetype,
  ...
  case ELostsenseEnemyArchetype::BishopPiston:
    if (BossPhase >= 2) {
      return {640.0, 24.0,   28.0,  2050.0, 315.0, 24.0,
              1.12,  245.0F, 0.50F, 0.70F,  0.80F};
    }
    return {640.0, 24.0,   28.0,  2050.0, 285.0, 20.0,
  ...
  FPortableEnemy(const uint64 Id, const ELostsenseEnemyArchetype Archetype)
      : Combatant{Lostsense::Combat::CombatantId{Id},
                  (Archetype == ELostsenseEnemyArchetype::Odran ||
                   ...
                   Archetype == ELostsenseEnemyArchetype::IlyrVael ||
                   Archetype == ELostsenseEnemyArchetype::AsterNull)
...
void ALostsenseEnemyCharacter::ConfigureEnemyArchetype(
  ...
  bBoss = InArchetype == ELostsenseEnemyArchetype::Odran ||
          ...
          InArchetype == ELostsenseEnemyArchetype::IlyrVael ||
          InArchetype == ELostsenseEnemyArchetype::AsterNull;
...
void ALostsenseEnemyCharacter::ConfigureGildedLungBoss(
    const uint64 InCombatantId, const uint32 InItemLevel) {
  ConfigureEnemyArchetype(InCombatantId, ELostsenseEnemyArchetype::GildedLung,
                          InItemLevel);
}
...
void ALostsenseEnemyCharacter::ConfigureAsterNullBoss(
    const uint64 InCombatantId, const uint32 InItemLevel) {
  ConfigureEnemyArchetype(InCombatantId, ELostsenseEnemyArchetype::AsterNull,
                          InItemLevel);
}
...
void ALostsenseEnemyCharacter::Tick(const float DeltaSeconds) {
  ...
  if ((Archetype == ELostsenseEnemyArchetype::GildedLung ||
    ...
    bBossFinalTransitionCommitted = true;
    BossPhase = 3;
    BossPulseCooldownRemaining = 0.9F;
    EnterState(ELostsenseEnemyAiState::Recover, 1.0F);
    return;
  }
...
void ALostsenseEnemyCharacter::HandleDefeat(
  ...
  const uint32 Table =
      Archetype == ELostsenseEnemyArchetype::Odran
          ? Lostsense::Gameplay::FirstSlice::OdranBossLoot.Value
          : (Archetype == ELostsenseEnemyArchetype::Kharos
  ...
  if (Story != nullptr) {
    ...
    } else if (Archetype == ELostsenseEnemyArchetype::BishopPiston) {
    ...
    } else if (Archetype == ELostsenseEnemyArchetype::IlyrVael) {
      static_cast<void>(Story->CompleteCampaignQuest(80074));
    } else if (Archetype == ELostsenseEnemyArchetype::AsterNull) {
      static_cast<void>(Story->CompleteCampaignQuest(80083));
...
bool ALostsenseEnemyCharacter::IsEncounterUnlocked() const {
  ...
  if (Archetype == ELostsenseEnemyArchetype::Caldris) {
  ...
  if (Archetype == ELostsenseEnemyArchetype::IlyrVael) {
    return Story->GetObjectiveState(80074) == ELostsenseObjectiveState::Active;
  }
  return Story->GetObjectiveState(80083) == ELostsenseObjectiveState::Active;
}
