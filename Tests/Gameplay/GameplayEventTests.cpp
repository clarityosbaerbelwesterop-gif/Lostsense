...
void TestStreamOrderingCapacityAndRollback(TestSuite &suite) {
  ...
  suite.Expect(stream.Publish(second) &&
                   stream.Events().back().Sequence == 2U,
  ...
  suite.Expect(stream.Publish(third) &&
                   stream.Events().back().Sequence == 3U,
  ...
  suite.Expect(!stream.Rollback({99U, 1U}),
...
void TestCommittedOperationsPublishOnlyAfterSuccess(TestSuite &suite) {
  ...
  suite.Expect(applied.Result == EffectApplyResult::Applied &&
                   events.Events().back().Type == GameplayEventType::EffectApplied,
...
void TestInventoryEquipmentAndDeathEventOrdering(TestSuite &suite) {
  ...
  suite.Expect(fixture.InventoryStateAuthority.AddInstance(
                   CommonInstance(ItemInstanceId{300U}, Sword)) ==
  ...
  suite.Expect(GameplayEventAuthority::Equip(
                   ...
                   ItemInstanceId{300U}, MainHand, fixture.Owner.Id(), events) ==
  ...
  suite.Expect(GameplayEventAuthority::Equip(
                   fixture.Equipment, fixture.InventoryStateAuthority,
                   ItemInstanceId{999U}, MainHand, fixture.Owner.Id(), events) !=
                   EquipmentResult::Success &&
                   events.Size() == afterEquip,
               "failed equip emits no event");

  suite.Expect(GameplayEventAuthority::Unequip(
  ...
  const auto damage = GameplayEventAuthority::ApplyDamage(
      fixture.Owner.Id(), enemy, 500.0, events);
